...
- **Content schema:** required `title, status: live|dead|paused, date, tags[], stack[], outcome`; optional `org_context, links{youtube,linkedin,live,repo,x,other}, hero_asset, cover_logo`. **Gated/test-only apps:** use `status: paused`, `links.other` with label `Request beta access` (Google Form `https://` URL preferred; `mailto:` until form exists), and the standard apply block in the case study — see `_template/` comments and doc 02 norm; the exact `Request beta access` label + mailto triggers the instructions modal (`src/lib/beta-access.ts`). Assets co-located in the project's `assets/` folder. Tags/stack normalized to lowercase slugs at compile time; build warns on single-use tags. Stack slugs with a known icon (`src/lib/stack-icons.ts`) render as brand-colored chips; unknown slugs fall back to plain text chips — the 15-minute contract never requires adding an icon.
